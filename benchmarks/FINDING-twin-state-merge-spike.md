...
2. **Re-scope the constraint** — allow a non-Bento writer (flow-core already has the correct CAS `MergeTelemetry` loop in `twinstore/nats.go`, but putting it in the hot path violates `CLAUDE.md`'s anti-pattern). Requires a rework of Phase 1 and an explicit constraint decision.
