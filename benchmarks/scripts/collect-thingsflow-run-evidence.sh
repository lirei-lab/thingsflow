#!/usr/bin/env bash
# Collects run evidence from a live deployment: RMQTT broker/node/client/plugin
# state, NATS stream and twin-KV state, history-store row counts, and filtered
# warning/error samples. JWTs and passwords are redacted before anything is
# written out.
#
# The history-store query is backend-dependent and both are supported:
#   HISTORY_SINCE_ISO  + GreptimeDB (default): HTTP SQL on :4000, /v1/sql?db=public
#   QUESTDB_SINCE_ISO  + QuestDB (optional backend): pg-wire on :8812
# Setting neither skips that section; everything else is backend-agnostic.
#
# This file previously carried a LEGACY header saying it was QuestDB-era and
# pending a GreptimeDB update. That header caused it to be deleted during a
# clean-up, which CI caught: 96% of it (177 of 185 lines) is backend-agnostic
# and is the only thing that collects broker and NATS evidence at all.
set -euo pipefail
...
HISTORY_SINCE_ISO="${HISTORY_SINCE_ISO:-}"
GREPTIME_SVC="${GREPTIME_SVC:-$RELEASE-greptimedb}"
GREPTIME_TABLE="${GREPTIME_TABLE:-device_telemetry_kv}"
...

{
  ...
  echo "history_since_iso=$HISTORY_SINCE_ISO"
  ...
  if [ -n "$HISTORY_SINCE_ISO" ]; then
    section "greptimedb history since window"
    kubectl -n "$NAMESPACE" exec "$RELEASE-postgres-0" -- \
      curl -s -m 30 "http://$GREPTIME_SVC:4000/v1/sql?db=public" \
      --data-urlencode "sql=SELECT count(DISTINCT device_id), count(*), max(ts) FROM $GREPTIME_TABLE WHERE ts > '$HISTORY_SINCE_ISO'" || true
  fi
